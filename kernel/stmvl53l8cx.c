...
static int stmvl53l8cx_i2c_probe(struct i2c_client *client)
{
	int ret;
...
static int stmvl53l8cx_spi_driver_probe(struct spi_device *pdev)
{
	...
	pr_info("%s: spi mode=%u, cs=%d, bits_per_word=%d, speed=%d, modalias=%s", 
						__func__, (unsigned int)pdev->mode, pdev->chip_select[0], pdev->bits_per_word, 
